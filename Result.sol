...
contract Result {
    ...
    function publishResult(address student, uint256 marks) public {
        require(!results[student].isPublished, "Result already published for this student");
        results[student] = ExamResult(marks, true);
    }

    function getMarks(address student) public view returns (uint256) {
        require(results[student].isPublished, "Result not published for this student");
