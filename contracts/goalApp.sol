...
contract goalApp {
    ...
    function contribute(uint256 _goalId, uint256 _amount) public {
        ...
        require(
            goals[_goalId].owner != msg.sender,
            "Owner cannot contribute to own goal"
        );
        require(
            goals[_goalId].current + _amount <= goals[_goalId].target,
    ...
    function withdraw(uint256 _goalId) public {
        require(
            goals[_goalId].deadline < block.timestamp,
            "Goal has not expired"
        );
